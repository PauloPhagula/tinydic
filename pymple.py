...
class Pymple(object):
    ...
    def register(self, key, value, constant=False):
        ...
        :param value: The value to be registered. Usually a function/lambda that takes the container as it's unique parameter
                      and returns a new object.
        :param constant: boolean flag indicating if the value should be registered as a constant or not. If False
                         then value must a callable which will be called with the container as unique parameter when
                         resolving the key.
        """
        if constant:
    ...
    def __setattr__(self, key, value):
        """
        Registes a new value
        :param key: The key used to resolve the value after registration
        :param value: The value to be registered. Usually a function/lambda that takes the container as it's unique parameter
                      and returns a new object.
                      If value is callable, then registration will be done as variable, otherwise will be as constant.
        """
        if callable(value):
